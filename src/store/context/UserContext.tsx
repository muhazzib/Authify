...
import { removeUserFromAsyncStorage, saveUserToAsyncStorage, saveUsersToAsyncStorage } from '../../utils/authStorage';
...
export const UserProvider = ({ children }: { children: ReactNode }) => {
    ...
    const login = (user: UserPayload, initialLoad?: boolean) => {
        ...
        setTimeout(() => {
            ...
            if (!existingUser || existingUser.password !== user.password) {
                if (!initialLoad) {
                    dispatch({ type: REDUCER_TYPES.SET_ERROR, payload: 'Please enter valid credentials.' });
                } else {
    ...
    const signup = (user: User) => {
        ...
        setTimeout(() => {
            const existingUser = state.users.find(u => u.email === user.email);
            if (existingUser) {
                dispatch({ type: REDUCER_TYPES.SET_ERROR, payload: 'User already exists. Please try a different email.' });
                setTimeout(() => {
                    dispatch({ type: REDUCER_TYPES.SET_ERROR, payload: null });
                }, 3000);
            } else {
